...
function doExplicitJoin(sql) {
  ...
  if (grpBy) {
    ...
    // BUG-4 fix: also support aliases like HAVING totaal > 2
    if (having) {
        ...
        rows = rows.filter(r => {
          ...
          if(key === undefined) {
            for (const p of colParts) {
              ...
              if (am && am[1].trim().toUpperCase().replace(/\s+/g,'') === havExprRaw) {
...
function doSingleSelect(sql) {
  ...
  // ORDER BY
  let orderBy = null;
  rest = rest.replace(/\s+order\s+by\s+(.+?)\s*$/i, (_, ob) => { orderBy = ob.trim(); return ''; }).trim();
    ...
    if(having) {
      ...
      if(hm) {
        ...
        gRows = gRows.filter(r => {
          ...
          if(!key) key = Object.keys(r).find(k => k.toUpperCase().replace(/\s+/g,'').includes(havExprRaw));
          if(!key && havExprRaw.includes('COUNT')) key = Object.keys(r).find(k => k === 'COUNT(*)');
          // Alias resolution: if aggregate was aliased in SELECT (e.g. COUNT(*) AS totaal),
          // the row key is the alias, not the raw expression. Look it up via colParts.
          if(key === undefined) {
            for (const p of colParts) {
              const am = p.match(/^(.+?)\s+as\s+(\w+)$/i);
              if (am && am[1].trim().toUpperCase().replace(/\s+/g,'') === havExprRaw) {
        ...
        // BUG-4: HAVING with alias (e.g. HAVING totaal > 2)
        const aliasName = aliasHm[1], op = aliasHm[2], n = Number(aliasHm[3]);
        gRows = gRows.filter(r => {
          // Match alias case-insensitively
          const key = Object.keys(r).find(k => k.toLowerCase() === aliasName.toLowerCase());
          if(key === undefined) return true;
          const v = Number(r[key]);
          switch(op){case'>':return v>n;case'<':return v<n;case'>=':return v>=n;case'<=':return v<=n;case'=':return v==n;case'!=':return v!=n;}
      ...
      colParts.forEach(p => {
        ...
        if (/^count\s*\(\s*\*\s*\)$/i.test(expr)) {
          ...
          if (aggM) {
            ...
            // Plain column (the GROUP BY key or other)
            const bare = expr.replace(/^\w+\./, '');
            out[label.replace(/^\w+\./, '')] = resolveCol(expr, grpRows[0]) ?? resolveCol(bare, grpRows[0]) ?? k;
          }
        }
      });
      return out;
